...
namespace mainApp
{
    ...
    {
        connect(m_pUi->m_pToolListView, SIGNAL(SignalCurrentIndexChanged(int)), this, SLOT(onCurrentIndexChanged(int)));
        connect(m_pUi->m_btnFront,      SIGNAL(clicked()),                      this, SIGNAL(sgnFrontWidget()));
        connect(m_pUi->m_btnExport,     SIGNAL(clicked()),                      this, SLOT(onExport()));
    }
    ...
    void RecipeElementWidget::onExport()
    {
        emit sgnExport();
    }

    void RecipeElementWidget::getElementListData(QList<QString>& listElement, QList<QString>& listUnit, QList<QString>& listNVR, QList<float>& listConstituent, float& eachPer)
    {
        m_pUi->m_pElementWidget->getElementListData(listElement, listUnit, listNVR, listConstituent, eachPer);
    }
